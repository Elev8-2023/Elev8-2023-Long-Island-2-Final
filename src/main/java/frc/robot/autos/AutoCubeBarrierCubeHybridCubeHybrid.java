...
public class AutoCubeBarrierCubeHybridCubeHybrid extends SequentialCommandGroup {
   

    public AutoCubeBarrierCubeHybridCubeHybrid(Swerve s_Swerve,Intake intake) {

        HashMap<String, Command> eventMap = new HashMap<>();
        
        List<PathPlannerTrajectory> pathgroup1 = PathPlanner.loadPathGroup("CubeBarrierCubeHybridCubeHybrid", new PathConstraints(3, 3));
        eventMap.put("Intake", new IntakeCube(intake, 75));
        eventMap.put("High Position", new CubePosition(intake, 75));
        eventMap.put("Shoot High", new ShootCube(intake, 0.9));
        eventMap.put("Intake 2", new IntakeCube(intake, 15));
        SwerveAutoBuilder autoBuilder = new SwerveAutoBuilder(
            
                s_Swerve::getPose, // Pose2d supplier
                s_Swerve::resetOdometry, // Pose2d consumer, used to reset odometry at the beginning of auto
                 Constants.Swerve.swerveKinematics, // SwerveDriveKinembatics
                new PIDConstants(3.9, 0.0, 0.0), // PID constants to correct for translation error (used to create the X
                                                 // and Y PID controllers)
                new PIDConstants(3.4, 0.0, 0.0), // PID constants to correct for rotation error (used to create the
                                                  // rotation controller)
        ...
        new CubePosition(intake, 42),
        new ShootCube(intake, 0.57),
        fullAuto1,
        eventMap.put("Hybrid Position", new CubePosition(intake, 75)),
        eventMap.put("Shoot High", new ShootCube(intake, 0.9))
